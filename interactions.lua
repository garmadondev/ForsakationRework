--[[
	🔧 interactions.lua (merged system - legacy-compatible, no InvokeServer)
	📦 Roblox Studio 2021 совместимость
	🎮 Comprehensive interaction system with fake server state
--]]
...
local clientRemotes, serverRemotes = remotes:WaitForChild("events"):WaitForChild("client"), remotes:WaitForChild("events"):WaitForChild("server")
local MainKeyInteraction = Enum.KeyCode.E

-- Libraries
local SC_Handcuffs = require(replicatedStorage.Modules.SC_Handcuffs)

-- 🧠 Локальное состояние (fake server state)
...
-- 🔄 Локальные имитации "InvokeServer"
...
local function fakeTriggerInteraction(player, obj)
	...
	if clientRemotes:FindFirstChild("InteractionResult") then
		clientRemotes.InteractionResult:FireClient(player, {
			success = true,
			object = obj.Name
		})
	end
...
local function client_dialog(obj)
	...
	if remotes.functions and remotes.functions.GameplayAction then
		remotes.functions.GameplayAction:InvokeServer(9, "Dialog", m)
	end
end

-- Client seek function
local function client_seek(model)
	if remotes.functions and remotes.functions.LootAction then
...
-- Interaction 1: Handcuffs/Detain
		...
		for i=1, #Parts do
			...
			if Parent:FindFirstChild("Humanoid") then
				...
				if TargetPlayer and SC_Handcuffs:CanDetain(Player, TargetPlayer) then
					...
					local handcuffsLibrary = serverScriptService:FindFirstChild("GameplayModules") and 
						serverScriptService.GameplayModules:FindFirstChild("HandcuffsLibrary")
					if handcuffsLibrary then
						require(handcuffsLibrary):Detain(Player, TargetPlayer)
					end
				end
			end
		end
	...
	if IsCheck then
		...
		if Keycard then
			...
			if clientRemotes:FindFirstChild("GunActionClient") then
				...
				clientRemotes.GunActionClient:FireClient(Who, 3, "КАРТОЧКА", "У вас своровали ключ-карту!", 10)
			end
...
-- Interaction 3: Steal guns (for Шизойды team)
...
-- Interaction 4: General interactions (storage, doors, dialog, seats)
interactions[4] = function(Player, Parts, IsCheck, ...)
	if IsCheck then		
		for i=1, #Parts do
			...
			if workspace:FindFirstChild("дурка") and Object:IsDescendantOf(workspace["дурка"].a1.doors) then
		...
		if Type == 1 then
			local doorModule = serverScriptService:FindFirstChild("Modules") and 
				serverScriptService.Modules:FindFirstChild("DoorModule")
			if doorModule then
				local Module = require(doorModule)
				...
				if Index and Module.Types then
...
-- Interaction 5: Extra flags and serve table
interactions[5] = function(Player, Parts, IsCheck, ...) -- Extra Flags.
	for i=1, #Parts do
		local obj = Parts[i]
...
function interactions.SpawnLootCrate(pos, lootType)
	...
	if lootCrateTemplate then
		...
		clone.Position = pos
		if clone:FindFirstChild("LootType") then
...
function interactions.AwardPlayer(player, rewardType)
	if rewardType == "TimeBonus" and timeModule then
		timeModule.AddTime(player, 10)
	elseif rewardType == "ScoreBoost" and recordModule then
...
if clientRemotes:FindFirstChild("TryInteract") then
	clientRemotes.TryInteract.OnServerEvent:Connect(function(player, obj)
		if obj then
